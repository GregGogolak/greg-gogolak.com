...
/**
 * Estimated P&L for an open position using current live price.
 * Buy-side costs only — sell fee and sell-day platform fee not yet incurred.
 * The actual close will use calculateTrade() with real numbers.
 */
export function calculateEstimatedPnL(position, livePrice) {
  const { entryPrice, shares, entryDate } = position

  const today = new Date().toISOString().split('T')[0]
  const a = new Date(entryDate)
  const b = new Date(today)
  const diffMs = Math.abs(b - a)
  const daysHeld = Math.floor(diffMs / (1000 * 60 * 60 * 24)) + 1

  const estimatedGross  = (livePrice - entryPrice) * shares
  const buyFee          = entryPrice * shares * 0.001
  const interestCost    = entryPrice * shares * 0.000212 * daysHeld
  const platformFee     = 96 // buy day only — sell day not yet incurred
  const totalCosts      = buyFee + interestCost + platformFee
  const netBeforeSplit  = estimatedGross - totalCosts
  const afterSplit      = netBeforeSplit * 0.50
  const afterTax        = afterSplit * 0.90
  const estimatedNetEur = afterTax * 0.90
