...
const BUCKET_C_PATTERNS = [
  ...
  /(nvidia|nvda).{0,60}(scrutin|investigat|probe|inquiry|lawmaker|congress|senate)/i,
  /(scrutin|investigat|probe).{0,60}(nvidia|nvda)/i,
]
...
const BUCKET_B_PATTERNS = [
  ...
  /(nvidia|nvda).{0,60}(launch|unveil|announc|open.source|model|platform|invest)/i,
  /(microsoft|google|amazon|meta|aws|azure|gcp).{0,80}(gpu|nvidia|ai|data.cent|cloud).{0,40}(expand|invest|secur|acquir|build|capex)/i,
  /(coreweave|hyperscaler).{0,60}(deal|contract|agreement|billion)/i,
  /ai.{0,30}(deal|contract|agreement|billion|invest)/i,
  /\$\d+\s*billion.{0,60}(ai|gpu|nvidia|data.cent)/i,
]
...
export function classifyWithKeywords(text) {
  ...
  // If headline mentions NVDA/Nvidia but no strong signal, classify as B (mild support)
  // rather than null — keeps NVDA-specific articles visible in SignalFeed
  const isNVDAMention = /nvidia|nvda/i.test(text)
  return isNVDAMention ? 'B' : null
}

/**
 * Scores a classified article on a 0-4 star scale.
 * Inputs: article must already have a bucket field (from classifyWithKeywords).
 *
 * @param {object} article - article with at least {headline, summary, datetime, bucket}
 * @returns {object} - original article + {score, stars, ageHours, freshness, specificity}
 */
export function scoreArticle(article) {
  const nowSec = Date.now() / 1000

  // Freshness weight
  const ageHours = (nowSec - article.datetime) / 3600
  const freshness = ageHours < 4 ? 1.0
                  : ageHours < 24 ? 0.5
                  : 0.1

  // Specificity weight
  const text = (article.headline + ' ' + (article.summary || '')).toLowerCase()
  const isNVDASpecific = /nvidia|nvda|jensen huang|blackwell|h100|h200|rubin/i.test(text)
  const isSector = /hyperscaler|data cent|capex|microsoft|google|amazon|meta|ai spend/i.test(text)
  const specificity = isNVDASpecific ? 1.0 : isSector ? 0.6 : 0.3

  // Type weight
  const typeWeight = article.bucket === 'C' ? 3
                   : article.bucket === 'B' ? 2
                   : 0

  const score = typeWeight * freshness * specificity

  // Star rating 0-4
  const stars = score === 0 ? 0
              : score < 0.5 ? 1
              : score < 1.5 ? 2
              : score < 2.5 ? 3
              : 4
  ...
  return {
    ...
    freshness,
    specificity: isNVDASpecific ? 'NVDA' : isSector ? 'SECTOR' : 'MACRO',
  }
}
