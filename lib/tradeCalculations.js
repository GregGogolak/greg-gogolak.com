 ...
 * Buy-day fee is split among users who bought that day; sell-day fee is split
 * among users who sold that day. Same-day round-trips count as both.
...
export function calculateSharedPlatformFee(trade, dateMap) {
  ...
  const buyUsers  = buyEntry?.buyUsers  ?? 1
  const sellUsers = sellEntry?.sellUsers ?? 1

  const buyDayFee      = 48 / buyUsers
  const sellDayFee     = 48 / sellUsers
