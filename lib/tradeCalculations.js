...
export const PER_EXECUTION_FEE = 48
export const FULL_UNSHARED_FEE = PER_EXECUTION_FEE * 2
...
export function calculateTrade({
  ...
  const buyDayFee  = buyDayAlreadyCharged ? 0 : PER_EXECUTION_FEE
  const sellDayFee = sellDayAlreadyCharged ? 0 : PER_EXECUTION_FEE
...
export function calculateSharedPlatformFee(trade, dateMap) {
  ...
  if (!dateMap) return {
    buyDayFee: PER_EXECUTION_FEE,
    sellDayFee: PER_EXECUTION_FEE,
    totalPlatformFee: FULL_UNSHARED_FEE,
    buyDaySharedWith: [],
    sellDaySharedWith: [],
    isSameDay,
    buyExecutions: 1,
  ...
  const buyDayFee      = PER_EXECUTION_FEE / buyExecutions
  const sellDayFee     = PER_EXECUTION_FEE / sellExecutions
...
export function recalculateNetWithSharedFees(trade, sharedFees) {
  const originalPlatformFee = trade.platform_fees_usd ?? (trade.buy_date !== trade.sell_date ? FULL_UNSHARED_FEE : PER_EXECUTION_FEE)
